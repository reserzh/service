...
import { updateEstimateFullAction, deleteEstimateAction, type EstimateActionState } from "@/actions/estimates";
...
export function EditEstimateForm({ estimate }: { estimate: EstimateData }) {
  const router = useRouter();
  const [isPending, setIsPending] = useState(false);
  const [isDeleting, setIsDeleting] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const [summary, setSummary] = useState(estimate.summary);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const result: EstimateActionState = await updateEstimateFullAction(estimate.id, {
      summary,
  ...
  async function handleDelete() {
    ...
    setIsDeleting(false);
    if (result.error) {
      showToast.error("Failed to delete estimate", result.error);
    } else {
      showToast.deleted("Estimate");
      ...
      <Card>
        ...
        <CardContent className="space-y-2 text-sm">
          ...
          <p>
            ...
            {estimate.property
              ? `${estimate.property.addressLine1}, ${estimate.property.city}, ${estimate.property.state}`
              : "Unknown"}
          </p>
        </CardContent>
      </Card>
        ...
        <AlertDialog>
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This will permanently delete estimate {estimate.estimateNumber}. This action cannot be undone.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
