...
export default function CreateEstimateScreen() {
  const draft = useEstimateDraftStore();
  ...
  useEffect(() => {
    (async () => {
      await draft.restore();
      const state = useEstimateDraftStore.getState();
      if (state.hasDraft && state.customerId) {
        setStep(state.step);
        setCustomerId(state.customerId);
        setCustomerName(state.customerName);
        setPropertyId(state.propertyId);
        setPropertyAddress(state.propertyAddress);
        if (state.options.length > 0) setOptions(state.options);
  ...
  useEffect(() => {
    if (customerId) {
      draft.setStep(step);
      draft.setCustomer(customerId, customerName);
      draft.setProperty(propertyId, propertyAddress);
      draft.setOptions(options);
      draft.setDetails(summary, notes, validUntil);
    }
