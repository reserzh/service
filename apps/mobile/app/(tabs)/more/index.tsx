...
function MenuItem({ icon, label, description, onPress, destructive }: MenuItemProps) {
  return (
    <Pressable
      onPress={onPress}
      className="flex-row items-center gap-4 bg-white dark:bg-slate-900 px-4 py-5 min-h-[64px] border-b border-slate-100 dark:border-slate-800 active:bg-slate-50 dark:active:bg-slate-800"
    >
  ...
  return (
    <SafeAreaView className="flex-1 bg-slate-50 dark:bg-slate-950" edges={["top"]}>
      ...
      <View className="rounded-xl overflow-hidden mx-4 mb-4">
        <MenuItem
          icon={<Users size={24} color="#64748b" />}
          label="Customers"
        ...
        />
        <MenuItem
          icon={<Receipt size={24} color="#64748b" />}
          label="Invoices"
